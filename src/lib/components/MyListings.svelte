<script lang="ts">
	...
	import { Plus, Eye, BarChart, Bolt, Pencil, Trash } from 'svelte-hero-icons';
	...
	const metrics = [
		...
		{ label: 'Total Leads', value: totalLeads.toLocaleString(), icon: BarChart },
		{ label: 'Active Listings', value: activeListings, icon: Bolt }
	];
						...
						<tr class="text-brand-text-primary">
							...
							<td class="p-4">
								<div class="flex items-center space-x-2">
									<Button size="sm" variant="ghost" on:click={() => openBoostModal(listing)}>
										<Bolt class="w-4 h-4" />
									</Button>
									<Button size="sm" variant="ghost">
										<Pencil class="w-4 h-4" />
									</Button>
									<Button size="sm" variant="ghost" class="text-red-500">
										<Trash class="w-4 h-4" />
									</Button>
								</div>
							</td>
