						...
						{#each listing.images as _, index}
							<div
								class={`h-2 rounded-full transition-all ${
									index === selectedImageIndex ? 'w-6 bg-white' : 'w-2 bg-white/50'
								}`}
							/>
						{/each}
					</div>
			...
			<div class="flex flex-col">
				<div>
					...
					<p class="text-sm text-brand-text-secondary mt-2">
						Listed by: <span class="font-semibold text-brand-text-primary"
							>{listing.vendorName}</span
						>
					</p>
				</div>
				...
				<div class="mt-auto pt-6">
					<Button on:click={handleContact} class="w-full !py-3 !text-lg justify-center">
						<ChatBubbleLeftRight class="h-6 w-6 mr-2" />
