...
<div class="space-y-6">
			...
			<div class="flex items-center gap-4">
				...
				<div class="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
					<button
						...
						aria-label="Grid View"><Squares2X2 class="h-5 w-5" /></button
					>
					<button
						...
						aria-label="List View"><Bars3 class="h-5 w-5" /></button
					>
				</div>
			</div>
		</div>
	...
	{:else}
		<Card class="text-center py-16">
			<InformationCircle class="h-12 w-12 text-gray-400 mx-auto mb-4" />
	...
	<Card class="flex flex-col md:flex-row justify-between items-center gap-4">
		...
		<div class="flex items-center gap-2">
			<Button
				...
				disabled={currentPage === 1}><ChevronLeft class="h-5 w-5" /></Button
			>
			<span class="text-sm font-medium">Page {currentPage} of {totalPages}</span>
			<Button
				variant="secondary"
				on:click={() => (currentPage = Math.min(totalPages, currentPage + 1))}
				disabled={currentPage === totalPages}><ChevronRight class="h-5 w-5" /></Button
			>
		</div>
	</Card>
</div>
